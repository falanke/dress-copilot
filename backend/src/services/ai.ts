...
export interface MultiImageRequest {
  images: string[]; // array of base64 or data URL images
  prompt: string;
}

export async function callZhipuMultiImage(apiKey: string, req: MultiImageRequest): Promise<VisionResponse> {
  console.log(`Calling Zhipu GLM-4.6V with ${req.images.length} images...`);
  ...
  try {
    ...
    console.error('Zhipu Multi-Image API Call Failed:', error);
    throw error;
  }
}
